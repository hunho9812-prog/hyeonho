...
export default function TodoPage() {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "planner", filter: `date=eq.${date}` }, async (payload) => {
        // 내가 방금 저장한 건지 debounce 타이머가 없을 때만 반영
        if (!saveTimerRef.current) {
          const newData = (payload.new as { data?: DayData }).data;
